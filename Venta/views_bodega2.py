...
from Producto.models import Producto2
from Cliente.models import Cliente
from Venta.models import Venta, DetalleVenta2
from .form import VentaForm, DetalleVentaForm2
...
def listado(request):
    ...
    return render(request, "Venta2/lista.html", {
        "page_obj": page_obj,
        "query": query,
...
def detalle(request, t):
    ...
    form = DetalleVentaForm2()
    detalles = DetalleVenta2.objects.filter(venta=venta).order_by('-fecha_actualizo')
    ...
    if request.method == 'POST':
        ...
        if 'buscar' in request.POST:
            termino = request.POST['buscar'].strip()
            if not termino:
                messages.error(request, 'Campo de búsqueda no puede estar vacío')
                return redirect('DetalleVenta2', t)

            querys = Q(codigo__icontains=termino) | Q(nombre__icontains=termino)
            busqueda = Producto2.objects.filter(querys)

            return render(request, 'Venta2/detalle.html', {
        ...
        # ➕ Agregar producto
            ...
            try:
                ...
                p = Producto2.objects.get(id=prod_id)   # ✅ instancia de Producto2
            except (ValueError, Producto2.DoesNotExist):
                messages.error(request, "Producto no encontrado")
                return redirect('DetalleVenta2', t)

            form = DetalleVentaForm2(request.POST)
            if form.is_valid():
                cantidad = int(form.cleaned_data['cantidad'])

                # Validar stock antes de modificar nada
                if cantidad > p.stock_actual:
                    messages.error(request, f'Este producto solo tiene {p.stock_actual} en stock!')
                    return redirect('DetalleVenta2', t)

                with transaction.atomic():
                    dt = DetalleVenta2.objects.filter(producto=p, venta=venta).first()
                    if dt:
                        # Acumular cantidad si ya existe
                        dt.cantidad += cantidad
                        dt.fecha_actualizo = datetime.today()
                        dt.save()
                        venta.total += cantidad * p.venta
                        venta.save()
                    else:
                        # Crear nuevo detalle
                        dt = form.save(commit=False)
                        dt.venta = venta
                        dt.producto = p   # ✅ asignar instancia, no p.id
                        dt.precio_unitario = p.venta
                        dt.cantidad = cantidad
                        dt.fecha_actualizo = datetime.today()
                        dt.save()
                        venta.total += p.venta * cantidad
                        venta.save()
        ...
        # ➖ Quitar producto
            ...
            try:
                dt = DetalleVenta2.objects.get(id=request.POST['cor_detalle'], venta=venta)
                pr = dt.producto
                pr.salida = max(0, pr.salida - dt.cantidad)
                pr.save()
                venta.total -= dt.cantidad * dt.precio_unitario
                venta.save()
                dt.delete()
            except DetalleVenta2.DoesNotExist:
        ...
        # ❌ Descartar venta completa
            ...
            with transaction.atomic():
                for dt in DetalleVenta2.objects.filter(venta=venta):
                    pr = dt.producto
                    pr.salida = max(0, pr.salida - dt.cantidad)
                    pr.save(update_fields=['salida'])
                DetalleVenta2.objects.filter(venta=venta).delete()
                venta.delete()
            messages.warning(request, "La venta fue descartada correctamente.")
            return redirect('NuevaVenta')

        # 📝 Actualizar datos de cliente según tipo de venta
        else:
    ...
    return render(request, 'Venta2/detalle.html', {
        'v': venta, 'form': form, 'detalle': detalles, 'c': cliente_existe
    })
...
def certificar_fel(request,venta):
    ...
    # Detalle de la venta
    for item in DetalleVenta2.objects.filter(venta=venta):
        num += 1
...
def anularfel(request, t):
    ...
    detalles = DetalleVenta2.objects.filter(venta=datoscliente)

    return redirect('ListaVenta2')
